...
@dag(
        ...
        schedule=DeltaTriggerTimetable(duration(days=3)), #This is a delta expression which means the DAG will run every 1 day
        end_date=datetime(year=2026, month=7, day=22,tz="UTC"),
        is_paused_upon_creation=False,
        catchup=True
)
def delta_scheduler_dag():
            @task
            def first_task():
