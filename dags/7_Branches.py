...
def branch_dag():
    ...
    #Creating the branching task to check if it is weekend or not
    @task.branch  #branch needs to be declared, otherwise no condition will not work 
    def decider_task(**kwargs):
        ti = kwargs['ti']
        weekend_flag = ti.xcom_pull(task_ids='extract_task')['weekend_flag']
        if weekend_flag == "true":
            print("It is weekend, skipping load task")
            return "no_load_task"
        else:
            print("It is not weekend, proceeding with load task")
            return "load_task"
    ...
    def no_load_task(**kwargs):
        ...
        return "echo 'No loading task on weekend'"
